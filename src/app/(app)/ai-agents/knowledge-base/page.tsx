    ...
    <div className="container mx-auto py-8 space-y-6">
      ...
      <div className="flex gap-4 mb-6">
        <Dialog open={openDialog === 'url'} onOpenChange={v => setOpenDialog(v ? 'url' : null)}>
          <DialogTrigger asChild>
            <Button className="group flex flex-col items-center justify-center w-32 h-24 gap-2 border bg-card shadow-none hover:bg-muted" variant="outline" onClick={() => setOpenDialog('url')}>
              <Globe className="w-6 h-6 group-hover:text-green-400 transition-colors" />
              <span className="font-medium text-sm group-hover:text-green-400 transition-colors">Add URL</span>
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-xl rounded-2xl p-0 overflow-hidden">
            ...
            <div className="flex flex-col gap-4 p-6">
              <div className="flex items-center gap-3 mb-2">
                <div className="bg-muted rounded-lg p-2"><Globe className="w-6 h-6" /></div>
        ...
        <Dialog open={openDialog === 'files'} onOpenChange={v => setOpenDialog(v ? 'files' : null)}>
          <DialogTrigger asChild>
            <Button className="group flex flex-col items-center justify-center w-32 h-24 gap-2 border bg-card shadow-none hover:bg-muted" variant="outline" onClick={() => setOpenDialog('files')}>
              <Upload className="w-6 h-6 group-hover:text-green-400 transition-colors" />
              <span className="font-medium text-sm group-hover:text-green-400 transition-colors">Add Files</span>
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-xl rounded-2xl p-0 overflow-hidden">
            ...
            <div className="flex flex-col gap-4 p-6">
              <div className="flex items-center gap-3 mb-2">
                <div className="bg-muted rounded-lg p-2"><Upload className="w-6 h-6" /></div>
                <span className="text-xl font-semibold">Add Files</span>
              </div>
              <div>
                <div
                  className="border-2 border-border border-dashed rounded-lg flex flex-col items-center justify-center py-8 mb-4 bg-card cursor-pointer"
                ...
                >
                  ...
                  <div className="text-muted-foreground text-xs mb-2">Up to 21 MB each.</div>
                  {file && <div className="text-xs text-foreground font-semibold mb-2">{file.name}</div>}
                  <div className="flex gap-1 flex-wrap justify-center">
                    {['epub','pdf','docx','txt','html'].map(type => (
                      <span key={type} className="bg-muted rounded-full px-2 py-0.5 text-xs font-medium text-foreground">{type}</span>
                    ))}
                  </div>
                </div>
                {fileUploading ? (
                  ...
                  <div className="w-3/4 bg-muted rounded-full h-2 mb-2">
                    ...
                    <div className="text-xs text-foreground">{uploadProgress}%</div>
                  </div>
                ) : (
                  <Button
                    className="bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm"
        ...
        <Dialog open={openDialog === 'text'} onOpenChange={v => setOpenDialog(v ? 'text' : null)}>
          <DialogTrigger asChild>
            <Button className="group flex flex-col items-center justify-center w-32 h-24 gap-2 border bg-card shadow-none hover:bg-muted" variant="outline" onClick={() => setOpenDialog('text')}>
              <FileText className="w-6 h-6 group-hover:text-green-400 transition-colors" />
              <span className="font-medium text-sm group-hover:text-green-400 transition-colors">Create Text</span>
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-xl rounded-2xl p-0 overflow-hidden">
            ...
            <div className="flex flex-col gap-4 p-6">
              <div className="flex items-center gap-3 mb-2">
                <div className="bg-muted rounded-lg p-2"><FileText className="w-6 h-6" /></div>
      ...
      <div className="flex items-center gap-2 mb-2">
        <Input
          ...
          className="w-[800px] bg-background border text-sm"
        />
        <DropdownMenu>
                  ...
                  >
                    <td className="flex items-center gap-3 px-6 py-4">
                      ...
                      {(() => {
                        ...
                        return Icon ? <Icon className="w-5 h-5 text-foreground group-hover:text-green-400 transition-colors flex-shrink-0" /> : null;
                      })()}
                      <div className="min-w-0 flex-1">
                        <div className="font-medium text-base flex items-center gap-2">
                          <span className="break-words group-hover:text-green-400 transition-colors">{article.name}</span>
                          ...
                          {article.type === "file" && article.file_path && (
                            <a
                              ...
                              className="ml-2 text-muted-foreground group-hover:text-green-400 transition-colors flex-shrink-0"
                              title="Download file"
                        ...
                        <div className="text-sm text-muted-foreground group-hover:text-green-400/80 transition-colors">{article.size}</div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <button className="bg-muted rounded-lg p-2 hover:bg-muted/80 focus:outline-none" onClick={e => e.stopPropagation()}>
                            <MoreHorizontal className="w-5 h-5" />
                          </button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent className="w-48 mt-2 rounded-lg border shadow-lg bg-card">
                          <DropdownMenuItem
