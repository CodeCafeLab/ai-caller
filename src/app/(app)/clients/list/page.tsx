...
function AllClientsListPageInner() {
  ...
  const [isEditClientSheetOpen, setIsEditClientSheetOpen] =
  ...
  const [managePlansForClient, setManagePlansForClient] =
    React.useState<null | { id: string; name: string }>(null);
    ...
    api
      ...
      .then(async (data) => {
        if (data.success) {
          ...
          const enhanced: Client[] = await Promise.all(
            processedData.map(async (cl: Client) => {
              try {
                const r = await api.getElevenLabsUsage(cl.clientId);
                const j = await r.json();
                if (j?.success && j?.data) {
                  const monthlyCalls =
                    typeof j.data.monthlyCalls === "number"
                      ? j.data.monthlyCalls
                      : cl.monthlyCallsMade;
                  const monthlyLimit =
                    typeof j.data.monthlyLimit === "number"
                      ? j.data.monthlyLimit
                      : cl.monthlyCallLimit;
                  const lifetime =
                    typeof j.data.lifetimeCalls === "number"
                      ? j.data.lifetimeCalls
                      : cl.totalCallsMade;
                  return {
                    ...cl,
                    monthlyCallsMade: monthlyCalls,
                    monthlyCallLimit: monthlyLimit,
                    totalCallsMade: lifetime,
                  };
                }
              } catch {}
              return cl;
  ...
  React.useEffect(() => {
    ...
    api
      ...
      .then((data) => {
        if (data.success)
          setPlans(data.data.map((p: any) => ({ id: p.id, name: p.name })));
      });
  ...
  React.useEffect(() => {
    if (managePlansForClient) {
      api
        .getAssignedPlansForClient(managePlansForClient.id)
        .then((res) => res.json())
        .then((data) =>
  ...
  const handleEditClientSuccess = async (formData: any) => {
    ...
    const formToSend: any = Object.fromEntries(
      Object.entries(formData).filter(([k]) => !blacklist.includes(String(k)))
    );
  ...
  const filteredClients = clients.filter((client) => {
    ...
    return (
      ...
      (planFilter === "all" ||
        client.plans.some((p) => p.toLowerCase() === planFilter))
    );
  });
  ...
  const handleAddClientSuccess = async (formData: AddClientFormValues) => {
    ...
    if (existingJson?.success && Array.isArray(existingJson.data)) {
      ...
      const exists = existingJson.data.some(
        (c: any) =>
          String(c.companyEmail || "")
            .trim()
    ...
    if (data.success) {
      ...
      toast({
        title: "Error",
        description: data.message || "Failed to add client",
        variant: "destructive",
      });
    }
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            <SheetContent
              className="sm:max-w-sm w-full flex flex-col"
              side="right"
            >
              <SheetHeader>
        ...
        <CardHeader className="border-b p-4">
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="flex gap-2 flex-wrap w-full md:w-auto justify-start items-center">
              ...
              <Popover
                open={planComboboxOpen}
                onOpenChange={setPlanComboboxOpen}
              >
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                  ...
                  >
                    ...
                    {dynamicPlanOptions.find((p) => p.value === planFilter)
            ...
            <TableBody>
              {paginatedClients.map((client) => (
                <TableRow key={client.id}>
                  <TableCell>
                    <Link
                      href={`/clients/details-usage?clientId=${client.clientId}`}
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium">
                          {client.monthlyCallsMade}
                        </span>
                        <span className="text-muted-foreground">
                          / {client.monthlyCallLimit}
                        </span>
                      </div>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {client.status === "Active" && (
                          <DropdownMenuItem
                            ...
                            onClick={() =>
                              handleStatusChange(client.clientId, "Suspended")
                            }
                          >
                        ...
                        {client.status === "Suspended" && (
                          <DropdownMenuItem
                            ...
                            onClick={() =>
                              handleStatusChange(client.clientId, "Active")
                            }
                          >
      ...
      >
        <SheetContent side="right" className="sm:max-w-sm w-full flex flex-col">
          <SheetHeader>
            <SheetTitle>
              Manage Plans
              {managePlansForClient ? ` – ${managePlansForClient.name}` : ""}
            </SheetTitle>
            <SheetDescription>
              View and remove assigned plans for this client.
            </SheetDescription>
          </SheetHeader>
              ...
              >
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    if (managePlansForClient) {
                      const r = await api.getAssignedPlansForClient(
                        managePlansForClient.id
                      );
                      const j = await r.json();
            ...
            >
              ...
              <div>
                ...
                <Input
                  name="companyEmail"
                  type="email"
                  value={editingClient.companyEmail || ""}
                  onChange={(e) =>
                    setEditingClient({
                      ...editingClient,
