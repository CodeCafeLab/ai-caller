...
const simplifyErrorMessage = (errorMessage: string): string => {
  ...
  if (
    error.includes("sip status: 480") ||
    error.includes("temporarily unavailable")
  ) {
    return "Phone is temporarily unavailable";
  }
  if (
    error.includes("sip status: 503") ||
...
const categorizeError = (
  ...
  if (
    conversation.call_successful === undefined ||
    conversation.call_successful === null
  ) {
    return {
      type: "Unknown Status",
  ...
  if (!conversation.call_successful) {
    if (conversation.error_message) {
      ...
      const simpleDescription = simplifyErrorMessage(
      ...
      if (
        ...
        errorMsg.includes("engaged") ||
        errorMsg.includes("486")
      ) {
        return {
          type: "Busy Signal",
          description: simpleDescription,
      ...
      if (
        errorMsg.includes("voicemail") ||
        errorMsg.includes("answering machine")
      ) {
        return {
          type: "Voicemail",
          description: simpleDescription,
      ...
      if (
        errorMsg.includes("audio quality") ||
        errorMsg.includes("echo") ||
        errorMsg.includes("feedback")
      ) {
        return {
      ...
      // TTS Errors
      if (
        errorMsg.includes("tts") ||
        errorMsg.includes("voice") ||
        errorMsg.includes("synthesis")
      ) {
        return {
          type: "TTS Error",
          description: simpleDescription,
    ...
    if (
      conversation.call_duration_seconds &&
      conversation.call_duration_seconds < 5
    ) {
      return {
        type: "No Answer",
  ...
  if (
    ...
    conversation.call_duration_seconds &&
    conversation.call_duration_seconds < 3
  ) {
    return {
      type: "Call Not Completed",
...
export default function ErrorLogsPage() {
  ...
  React.useEffect(() => {
    async function fetchErrorLogs() {
      ...
      try {
          ...
          try {
            ...
            if (testResp.ok) {
              ...
              console.log(
                ...
                testJson.conversations?.length || 0
              );
            } else {
        ...
        const allConversations = conversationsData.filter(
          (conv) =>
            ...
            (conv.call_successful &&
              ...
              conv.call_duration_seconds < 3) ||
            // Temporarily include all calls for debugging
            true
        );

        console.log(
          "Filtered conversations for error logs:",
          allConversations.length
        );
        console.log(
          "Sample filtered conversation:",
          allConversations.slice(0, 2)
        );
        ...
        const detailedConversations = await Promise.all(
          allConversations.map(async (conv) => {
            try {
              const detailResponse = await elevenLabsApi.getConversationDetails(
              ...
              if (detailResponse.ok) {
                ...
                return {
                  ...
                  failure_reason:
                    detailData.failure_reason || detailData.error_message,
                  call_status: detailData.call_status || "Failed",
                };
              }
            } catch (error) {
              console.error(
                `Error fetching details for conversation ${conv.conversation_id}:`,
                error
              );
            }
            return conv;
        ...
        console.log(
          ...
          detailedConversations.length
        );
        console.log("Sample detailed conversation:", detailedConversations[0]);
        const errorLogsData: ErrorLogEntry[] = detailedConversations.map(
          (conv, index) => {
            ...
            return {
              ...
              timestamp: (() => {
                try {
                  ...
                  if (unixTimestamp) {
                    ...
                    console.log(
                      "Created timestamp for conversation",
                      conv.conversation_id,
                      ":",
                      date.toISOString(),
                      "from field:",
                    ...
                    console.warn(
                      "No valid timestamp field found for conversation:",
                      conv.conversation_id,
                      "Available fields:",
                      Object.keys(conv)
                    );
              ...
              callStatus:
                conv.call_status ||
                (conv.call_successful ? "Success" : "Failed"),
        ...
        const validErrorLogs = errorLogsData.filter(
          (log) => log && log.id && log.timestamp
        );
      ...
      const dateMatch =
        dateRange?.from && dateRange?.to
          ? log.timestamp >= dateRange.from &&
            log.timestamp <= addDays(dateRange.to, 1)
          : true;

      // Agent filtering - match specific agent or show all
      const agentMatch =
        selectedAgentId === "all" || log.agentId === selectedAgentId;

      // Error type filtering - match specific error type or show all
      const errorTypeMatch =
        selectedErrorType === "all" || log.errorType === selectedErrorType;
  ...
  const handleApplyFilters = () => {
    ...
    toast({
      ...
      description: "Error log data has been updated.",
    });
  };
  ...
  const handleResetFilters = () => {
    ...
    toast({
      title: "Filters Reset",
      description: "Error log filters have been reset.",
    });
  };
  ...
  const handleExport = (format: "CSV" | "Excel" | "PDF") => {
    ...
    console.log(
      `Simulating export of error logs as ${format}. Data:`,
      filteredErrorLogs
    );
  };
  ...
  const handleMarkResolved = (logId: string) => {
    ...
    toast({
      ...
      description: `Error log marked as ${
        newStatus === "Yes" ? "resolved" : "unresolved"
      }.`,
    });
  };
  ...
  const failureRate = React.useMemo(() => {
    ...
    const criticalErrors = filteredErrorLogs.filter(
      (log) => log.errorType !== "No Answer"
    ).length;

    return totalCalls > 0
      ? ((criticalErrors / totalCalls) * 100).toFixed(1) + "%"
      : "0%";
  }, [filteredErrorLogs, usageStats]);
  const commonErrorType =
    filteredErrorLogs.length > 0
      ? Object.entries(
          filteredErrorLogs.reduce((acc, log) => {
            acc[log.errorType] = (acc[log.errorType] || 0) + 1;
            return acc;
          }, {} as Record<ErrorType, number>)
        ).sort((a, b) => b[1] - a[1])[0]?.[0] || "N/A"
      : "N/A";
  const affectedAgentsCount = new Set(
    filteredErrorLogs.map((log) => log.agentId)
  ).size;
    ...
    <div className="container mx-auto py-8 space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold font-headline flex items-center">
            <ShieldAlert className="mr-3 h-8 w-8 text-destructive" /> Call
            Reports & Error Logs
          </h1>
          <p className="text-muted-foreground">
            View all call reports including failed calls, unknown status calls,
            and issues. Error messages are shown in simple language.
          </p>
        </div>
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 items-end">
          ...
          <div className="flex flex-col space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setSelectedErrorType(value as ErrorType | "all")
              }
            >
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{affectedAgentsCount}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{filteredErrorLogs.length}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            List of all call reports including failed calls, unknown status
            calls, and issues. Error messages are displayed in simple,
                        ...
                        <TableRow key={log.id}>
                          ...
                          <TableCell>
                            <Badge
                              className={cn(
                                "text-xs",
                                resolvedStatusColors[log.isResolved]
                              )}
                            >
                              {log.isResolved}
                            </Badge>
        ...
        <CardFooter className="p-4 border-t flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="text-xs text-muted-foreground">
            ...
            -{" "}
            <strong>
              {Math.min(currentPage * itemsPerPage, filteredErrorLogs.length)}
            </strong>{" "}
            of <strong>{filteredErrorLogs.length}</strong> logs
          </div>
          <div className="flex gap-2">
            <Button
              variant="outline"
