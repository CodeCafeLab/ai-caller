...
export default function AiAgentsPage() {
  ...
  const [languageFilter, setLanguageFilter] = React.useState<
    AIAgentLanguage | "all"
  >("all");
  const [statusFilter, setStatusFilter] = React.useState<AIAgentStatus | "all">(
    "all"
  );
  ...
  const refreshAgents = React.useCallback(async () => {
    ...
    try {
      ...
      let allClients: any[] = Array.isArray(clientsData.data)
        ? clientsData.data
        : [];
      ...
      if (Array.isArray(localData.data)) {
        localAgents = localData.data.map((agent: any) => {
          const client = allClients.find(
            (c: any) => String(c.id) === String(agent.client_id)
          );
          return {
            id: agent.agent_id,
            name: agent.name,
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mergedData = {
          ...
          system_prompt:
            data.elevenlabs?.conversation_config?.agent?.prompt?.prompt ||
            data.local?.system_prompt,
          language_code:
            data.elevenlabs?.conversation_config?.agent?.language ||
            data.local?.language_code,
          llm:
          ...
          voice_id:
            data.elevenlabs?.conversation_config?.tts?.voice_id ||
            data.local?.voice_id,
          model: data.elevenlabs?.model || data.local?.model,
          tags:
            data.elevenlabs?.tags ||
            (data.local?.tags ? JSON.parse(data.local.tags) : []),
  ...
  console.log(
    "agent.client_id values:",
    agents.map((a) => [a.client_id, typeof a.client_id])
  );

  // In the render function, filter by client_id and then apply search/filters
  const filteredAgents = agents
    .filter((agent) => String(agent.client_id) === String(user?.userId))
    .filter((agent) => {
      ...
      const matchesUseCase =
        useCaseFilter === "all" || agent.useCase === useCaseFilter;
      const matchesLanguage =
        languageFilter === "all" || agent.language === languageFilter;
      const matchesStatus =
        statusFilter === "all" || agent.status === statusFilter;
      return (
        matchesSearch && matchesUseCase && matchesLanguage && matchesStatus
      );
    });
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage global AI conversation agents.
          </p>
        </div>
      ...
      <Card className="shadow-lg">
        <CardHeader className="border-b p-4">
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="flex gap-2 flex-wrap w-full md:w-auto justify-start items-center">
              <Popover
                open={useCaseComboboxOpen}
                onOpenChange={setUseCaseComboboxOpen}
              >
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {useCaseOptions.find((uc) => uc.value === useCaseFilter)
                ...
                <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                  <Command>
                    ...
                    <CommandList>
                      <CommandEmpty>No use case.</CommandEmpty>
                      <CommandGroup>
                        {useCaseOptions.map((opt) => (
                          <CommandItem
                            ...
                            value={opt.label}
                            onSelect={() => {
                              setUseCaseFilter(opt.value);
                              setUseCaseComboboxOpen(false);
                            }}
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                useCaseFilter === opt.value
                                  ? "opacity-100"
                                  : "opacity-0"
                              )}
                            />
              ...
              >
                <PopoverTrigger asChild>
                  ...
                  >
                    <ListFilter className="mr-2 h-4 w-4 shrink-0 opacity-50" />{" "}
                    {}
                    {statusFilterOptions.find((st) => st.value === statusFilter)
                  ...
                  paginatedAgents.map((agent) => (
                    <TableRow key={agent.id}>
                      <TableCell className="font-medium">
                        <Link
                          href={`/client-admin/ai-agents/create/details/${agent.id}`}
                              ...
                              setAgents((prev) =>
                                prev.map((a) =>
                                  a.id === agent.id
                                    ? {
                                        ...a,
                                        status: newStatus as AIAgentStatus,
                                      }
                                    : a
                                )
                              );
                          ...
                          >
                            ...
                            <SelectContent className="p-0 min-w-[70px] max-w-[110px]">
                              <SelectItem value="Published">
                                <span
                                  className={cn(
                                    "rounded-full px-2 py-0 text-xs font-semibold h-5 min-h-0 text-center whitespace-nowrap overflow-visible",
        ...
        <CardFooter className="p-4 border-t flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="text-xs text-muted-foreground">
            ...
            of <strong>{filteredAgents.length}</strong> agents
          </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div>
                      ...
                      {viewAgent.temperature || "N/A"}
                    </div>
                    <div>
                      <span className="font-medium">Voice ID:</span>{" "}
                      {viewAgent.voice_id || "N/A"}
                    </div>
                    <div>
              ...
              {viewAgent.tags && viewAgent.tags.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Array.isArray(viewAgent.tags) ? (
                      viewAgent.tags.map((tag: string, index: number) => (
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  onClick={() => {
                    ...
                    handleEdit({
                      id: viewAgent.agent_id || viewAgent.id,
                    } as AIAgent);
                  }}
                >
                  Edit Agent
