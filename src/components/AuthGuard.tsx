      ...
      try {
        ...
        if (storedUser && storedToken) {
          try {
            ...
            setIsLoading(false);
            console.log(
              "[AuthGuard] User restored from localStorage, authentication complete"
            );
            return;
          } catch (error) {
            console.warn(
              "[AuthGuard] Failed to parse stored user data:",
              error
            );
        ...
        try {
          ...
          console.log(
            "[AuthGuard] API response headers:",
  ...
  useEffect(() => {
    if (
      ...
      const hasTypePermission =
        ...
        (user.type && allowedTypes.includes(user.type));
