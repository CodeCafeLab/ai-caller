...
const campaignTypes = ["Outbound", "Follow-Up", "Reminder"] as [
  CampaignType,
  ...CampaignType[]
];
...
interface AddCampaignFormProps {
  ...
  onSuccess: (
    data: Omit<Campaign, "id" | "callsAttempted" | "status" | "successRate">
  ) => void;
  onCancel: () => void;
}
...
export function AddCampaignForm({
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        const agentsUrl = urls.backend.campaigns.agents(
          currentClientId ? String(Number(currentClientId)) : undefined
        );
        ...
        if (!agentsJson) {
          try {
            const rel = currentClientId
              ? `/api/campaigns/agents?client_id=${encodeURIComponent(
        ...
        const phoneList = Array.isArray(phonesJson?.phone_numbers)
          ? phonesJson.phone_numbers
          : Array.isArray(phonesJson?.items)
          ? phonesJson.items
          : Array.isArray(phonesJson)
          ? phonesJson
          : [];
        setPhoneNumbers(
          phoneList.map((p: any) => {
            const id = String(
              p.id || p.phone_number_id || p.number || p.e164 || p.uuid || p._id
            );
  ...
  async function onSubmit(data: AddCampaignFormValues) {
    ...
    if (sheetFile) {
      ...
      form.append(
        "agent_name",
        agents.find((a) => a.id === data.agentId)?.name || "Unknown Agent"
      );
      ...
      if (scheduleMode === "now") {
        ...
        form.append(
          "scheduled_time_unix",
          String(Math.floor(new Date(data.startDate).getTime() / 1000))
        );
      }
        ...
        <ScrollArea className="flex-1 min-h-0">
          <div className="space-y-4 px-2 py-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="e.g., leadgen, q1, urgent"
                      {...field}
                      className="h-9 text-sm"
                    />
                  </FormControl>
                  ...
                  >
                    ...
                    <SelectContent className="max-h-60 overflow-y-auto">
                      {Array.isArray(agents) && agents.length > 0 ? (
                        ...
                        <SelectItem disabled value="__no_agents__">
                          No agents for this client
                        </SelectItem>
                      )}
                    </SelectContent>
                  </Select>
            ...
            <div className="space-y-2">
              ...
              />
              <FormDescription>
                Upload a CSV/XLSX with ElevenLabs format: phone_number,
                language, voice_id, first_message, prompt, city,
                other_dyn_variable
              </FormDescription>
            </div>
