    ...
    <section id="how-it-works" className="py-20 md:py-28">
      <div className="container">
        ...
        <div className="relative mt-12 grid grid-cols-1 gap-8 md:grid-cols-4 md:gap-4">
          <div className="absolute top-8 left-0 hidden w-full h-px bg-border -translate-y-1/2 md:block" aria-hidden="true" />
          {steps.map((step, index) => (
            <div key={index} className="relative flex flex-col items-center p-4 text-center">
               <div className={`absolute -top-4 left-1/2 w-px h-4 bg-border -translate-x-1/2 md:hidden ${index === 0 ? 'hidden' : ''}`} aria-hidden="true" />
              <div className="z-10 flex h-16 w-16 items-center justify-center rounded-full border-2 border-primary bg-background">
                {step.icon}
              </div>
