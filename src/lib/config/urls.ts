...
export const urls = {
  ...
  elevenlabs: {
    ...
    agentConversations: (agentId: string) =>
      `${config.elevenlabs.convai.agents}/${agentId}/conversations`,
    agentDuplicate: (agentId: string) =>
      `${config.elevenlabs.convai.agents}/${agentId}/duplicate`,
    agentLink: (agentId: string) =>
      `${config.elevenlabs.convai.agents}/${agentId}/link`,
    agentSettings: (agentId: string) =>
      `${config.elevenlabs.legacy.agents}/${agentId}/settings`,
    agentWidgetConfig: (agentId: string) =>
      `${config.elevenlabs.legacy.agents}/${agentId}/widget-config`,
    knowledgeBase: {
      ...
      document: (docId: string) =>
        `${config.elevenlabs.convai.knowledgeBase}/${docId}`,
      content: (docId: string) =>
        `${config.elevenlabs.convai.knowledgeBase}/${docId}/content`,
      dependentAgents: (docId: string) =>
        `${config.elevenlabs.convai.knowledgeBase}/${docId}/dependent-agents`,
    },
    secrets: {
      create: config.elevenlabs.convai.secrets,
      update: (secretId: string) =>
        `${config.elevenlabs.convai.secrets}/${encodeURIComponent(secretId)}`,
  ...
  backend: {
    ...
    campaigns: {
      ...
      listForClient: (clientId: string) =>
        `${config.backend.api}/campaigns/client/${encodeURIComponent(
          clientId
        )}`,
      details: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}`,
      localDetails: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}/local`,
      recipients: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}/recipients`,
      testBatch: (id: string) =>
        `${config.backend.api}/campaigns/test-batch/${encodeURIComponent(id)}`,
      cancel: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}/cancel`,
      retry: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}/retry`,
      ...
      pause: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}/pause`,
      resume: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}/resume`,
      patch: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}`,
      delete: (id: string) =>
        `${config.backend.api}/campaigns/${encodeURIComponent(id)}`,
      phoneNumbers: () => `${config.backend.api}/campaigns/phone-numbers`,
      agents: (clientId?: string) =>
        clientId
          ? `${
              config.backend.api
            }/campaigns/agents?client_id=${encodeURIComponent(clientId)}`
          : `${config.backend.api}/campaigns/agents`,
    },
  },
};
